shinyServer(function(input, output, session) {  
  ...
  output$DaysLeft<-renderUI({
    ...
    sliderInput(inputId = "DaysLeft",label = NULL,post = " Days Left",
                min = xmin,max = xmax,step=1,value=50)
  })
  
  selectedData <- reactive({
    a=x%>%filter(Election%in%input$Election&Date>=input$daterange[1]&Date<=input$daterange[2]&!is.na(Mandates)&DaysLeft<=input$DaysLeft)
...
#     a=x%>%count(Election,input$varx,input$vary,input$fill_var)%>%filter(n>1)}
    ...
    p=ggplot(a)+theme_bw()+theme(axis.text.x = element_text(angle = 90*as.numeric("Rotate Label"%in%input$axis.attr)))+xlab(input$varx)
